...
- [ ] 3. Slice 2 — Routing: EventBridge rule and Webhook Lambda
  ...
  - [x] 3.9 Scaffold the browser-based page-load/UI canary (Requirement 12.2/12.3)
    ...
    - Fixed a second time after live re-validation surfaced two false-positive classes: (a) the adoption-list page's `.pet-item` check failed whenever nobody had adopted anything yet, even though a correctly-rendered "No adoption data available" (`.alert-warning`) empty state is not an error — added a per-page `emptyStateSelector` so a configured valid empty state is treated as OK; (b) PetSite's own template always renders a hidden, empty `.alert-danger` placeholder (populated by client-side JS only on a real error), so mere DOM presence of the error selector was a guaranteed false positive on the Buy Food page — changed the secondary error-indicator check to require visibility (not `display:none`/`visibility:hidden`/`opacity:0`/zero-size), not just DOM presence
